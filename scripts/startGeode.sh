...
STD_SERVER_ITEMS="${STD_SERVER_ITEMS} --user=super-user --password=1234567"
...
function launchLocator() {
    ...
    gfsh -e "start locator --hostname-for-clients=${HOSTNAME} --J=-Dpulse.useLocator=true  --bind-address=${HOSTNAME} --security-properties-file=${APP_HOME}/etc/gfsecurity.properties --initial-heap=${DEFAULT_LOCATOR_MEMORY} --max-heap=${DEFAULT_LOCATOR_MEMORY} ${DEFAULT_JVM_OPTS} --name=locator$1_`hostname` --port=1033$1 --dir=${APP_HOME}/data/locator$1 --locators=${LOCATORS} --classpath=${APP_HOME}/etc --J=-Dgemfire.security-password=1234567 --J=-Dgemfire.security-username=super-user --J=-Dgemfire.security-manager=org.apache.geode.examples.security.ExampleSecurityManager" &
...
gfsh -e "connect --locator=${LOCATORS} --user=super-user --password=1234567 --security-properties-file=${APP_HOME}/etc/gfsecurity.properties" -e "configure pdx --read-serialized=true --disk-store=DEFAULT"
...
gfsh -e "connect --locator=${LOCATORS} --user=super-user --password=1234567 --security-properties-file=${APP_HOME}/etc/gfsecurity.properties" -e "create region --name=test --type=PARTITION"
